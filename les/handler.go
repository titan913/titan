...
// Package les implements the Light gee Subprotocol.
package les
...
// NewProtocolManager returns a new william sub protocol manager. The gee sub protocol manages peers capable
// with the william network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light gee protocol")
	...
	log.Info("Light gee protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light gee peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light gee handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light gee peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light gee message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light gee message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the gee sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
