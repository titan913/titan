...
import (
	...
	"github.com/william/gee/consensus/misc"
...
func (self *worker) update() {
	...
	if self.config.Dpos != nil && self.config.Dpos.Period > 0 {
		dposDelay = time.Duration(self.config.Dpos.Period) * time.Second
	}
	...
	for {
		...
		case ev := <-self.chainSideCh:
			if self.config.Dpos != nil {
				// uncle block useless in Dpos consensus
...
func (self *worker) commitNewWork() {
	...
	self.push(work)
	self.updateSnapshot()
	// todo: add params into gee, to decide if or not send this tx
	if self.config.Dpos != nil && self.config.Dpos.PBFTEnable {
...
// For PBFT of dpos consensus, the follow code may move later
// After confirm a new block, the miner send a custom transaction to self, which value is 0
// and data like "gee:1:event:confirm:123" (gee is prefix, 1 is version, 123 is block number
		...
		if len(wallet.Accounts()) == 0 {
			...
			for _, account := range wallet.Accounts() {
				if account.Address == self.coinbase {
					...
					tmpTx := types.NewTransaction(nonce, account.Address, big.NewInt(0), uint64(100000), big.NewInt(10000), []byte(fmt.Sprintf("gee:1:event:confirm:%d", blockNumber)))
